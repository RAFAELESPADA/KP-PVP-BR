  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
  {
    ...
    if (args.length == 1)
    {
      if (args[0].equalsIgnoreCase("0"))
      {
        ...
        player.sendMessage(API.NomeServer + "Seu gamemode foi atualizado para survival.");
      }
      else if (args[0].equalsIgnoreCase("1"))
      {
        player.setGameMode(GameMode.CREATIVE);
        player.sendMessage(API.NomeServer + "Seu gamemode foi atualizado para criativo.");
        Bukkit.broadcast(API.NomeServer + "§a" + player.getName() + ChatColor.GRAY + " Entrou no gamemode 1!", "kitpvp.staff");
      }
      else if (args[0].equalsIgnoreCase("2"))
      {
        player.setGameMode(GameMode.ADVENTURE);
        player.sendMessage(API.NomeServer + "Seu gamemode foi atualizado para adventure.");
      }
      else
      {
    ...
    if (args.length == 2 && player.hasPermission("kitpvp.dargm"))
    {
      ...
      if (args[0].equalsIgnoreCase("0"))
      {
        ...
        target.sendMessage(API.NomeServer + "Voce entrou no modo §asurvival.");
      ...
      else if (args[0].equalsIgnoreCase("1"))
      {
        ...
        target.sendMessage(API.NomeServer + "Voce entrou no modo criativo.");
      ...
      else if (args[0].equalsIgnoreCase("2"))
      {
        ...
        target.sendMessage(API.NomeServer + "Voce entrou no modo adventure.");
